...
const categoryFromPath = (path: string[]) =>
  path.length ? path[path.length - 1] : "Gallery";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!event.drive_folder_id) {
      throw new Error(
        "This event has no Google Drive folder selected."
      );
    }
    ...
    async function scanFolder(
      ...
      do {
        const response = await drive.files.list({
          q: `'${parentId.replaceAll(
            "'",
            "\\'"
          )}' in parents and trashed=false`,
          fields:
            "nextPageToken,files(id,name,mimeType,imageMediaMetadata,createdTime,modifiedTime,size)",
          pageSize: 1000,
          pageToken,
        });
    ...
    const staleIds = (existing || [])
      .filter(
        (p: any) =>
          !discovered.has(p.drive_file_id)
      )
      .map((p: any) => p.id);
